...
struct mem_ret sev_realloc(void **original, size_t size){
    if(!original || !*original || size < 1) return (struct mem_ret){ NULL, BAD_PARAM };
    ...
    if(!allocated){
        ...
        if(errno == ENOMEM){
            printf("%s\n", strerror(errno));
            return (struct mem_ret){ NULL, NO_MEM };
        }
        return (struct mem_ret){ NULL, UNSPECIFIED };
    }
    return (struct mem_ret){ allocated, OK };
}

struct mem_ret sev_calloc(size_t nmemb, size_t size){
    if(nmemb < 1 || size < 1) return (struct mem_ret){ NULL, BAD_PARAM };
...
struct mem_ret sev_malloc(size_t size){
    if(size < 1) return (struct mem_ret){ NULL, BAD_PARAM };
...
struct codepoint_ret bytestr_to_codepoints(char *const bytestr, const size_t len){
    if(!bytestr || len < 1) return (struct codepoint_ret){ NULL, 0, BAD_PARAM };
   
    // Allocate the codepoint buffer (used for editing and rendering)
    struct mem_ret alloc_result = sev_calloc(len, sizeof(i32));
    if(alloc_result.err != OK){
        sev_free(bytestr);
        return (struct codepoint_ret){ NULL, 0, alloc_result.err };
    }
    i32 *buffer = (i32 *)alloc_result.allocated; 

    struct codepoint_fill_ret result = { 0, 0, 0, UNSET };
    ...
    if(result.err != OK || result.jpos < 1) {
        sev_free(bytestr);
        sev_free(buffer);
        return (struct codepoint_ret){ NULL, len, result.err };
    }
    // Done with this
    sev_free(bytestr);
    ...
    if(result.jpos < pdif_pread){
        ...
        if(alloc_result.err != OK){
            sev_free(buffer);
            return (struct codepoint_ret){ NULL, uj, alloc_result.err };           
        }
        buffer = (i32 *)alloc_result.allocated;
    }

    return (struct codepoint_ret){ buffer, uj, OK };
}
...
struct codepoint_fill_ret codepoint_buffer_fill(const char *const bytestr, i32 *const buffer, const ptrdiff_t pread){
    ...
    while(i < pread){
        ...
        if(n <= 0){
            return (struct codepoint_fill_ret){ n, j, i, UTF8_ITERATE_BAD_RET };
        }
        i += n, j++, last_consumed = n;
    }
    return (struct codepoint_fill_ret){ last_consumed, j, i, OK };
}
